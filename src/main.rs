...
#[derive(Parser, Debug)]
#[clap(version = "0.1", about = "Gets and (aspirationally) sets the time.")]
struct ClockArgs {
    /// Action to perform
    #[clap(default_value = "get")]
    action: Action,

    /// Time standard to use
    #[clap(short, long, default_value = "rfc3339")]
    std: Standard,

    /// Datetime value (only used when action is 'set')
    #[clap(help = "When <action> is 'set', apply <datetime>. Otherwise, ignore.")]
...
impl From<NTPTimestamp> for DateTime<Utc> {
    fn from(ntp: NTPTimestamp) -> Self {
        ...
        let timestamp = Utc.timestamp_opt(secs, nanos as u32);
        timestamp.unwrap()
    }
}
...
fn main() {
    let args = ClockArgs::parse();

    let action = args.action;

    if action == Action::Set {
        let t_ = args.datetime.unwrap();

        let parser = match args.std {
            Standard::RFC2822 => DateTime::parse_from_rfc2822,
            Standard::RFC3339 => DateTime::parse_from_rfc3339,
            _ => unimplemented!(),
        };

        let err_msg = format!("Unable to parse {} according to {:?}", t_, args.std);
